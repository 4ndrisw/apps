...
class Dashboards extends AdminController
{
  ...
  public function my_dashboard()
  {
    ...
    $user_dashboards = $this->apps_model->get_dashboards_by_user($user->staffid);
  ...
  public function index()
  {
    ...
    $dashboards = $this->apps_model->get_dashboards();
    $data['dashboards'] = $dashboards;

    $data['title'] = _l('apps_dashboard_index_title_page');
    $this->load->view('dashboards/index', $data);
  }

  public function edit_dashboard()
  {
    if (!has_permission('apps', '', 'dashboard_edit')) {
      access_denied('apps');
    }
    $dashboard_id = $this->input->get('dashboard_id');
    if (!isset($dashboard_id)) {
    ...
    $dashboard_rows = $this->apps_model->find_dashboard($dashboard_id);
    ...
    $dashboard_staff = $this->apps_model->get_dashboard_staff($dashboard['id']);
    $data['dashboard_staff'] = $dashboard_staff;

    $data['title'] = _l('apps_edit_dashboard_title_page', $dashboard['name']);
    $this->load->view('dashboards/edit', $data);
  }

  public function store_dashboard()
  {
    if (!has_permission('apps', '', 'dashboard_create')) {
      access_denied('apps');
    }
    ...
    $dashboard_id = $this->apps_model->create_dashboard($dashboard_name, $note);

    set_alert('success', _l('apps_message_success_create_dashboard'));
    redirect(admin_url('apps/dashboards'));
  }

  public function clone_dashboard()
  {
    if (!has_permission('apps', '', 'dashboard_clone')) {
      access_denied('apps');
    }
    if ($this->input->server('REQUEST_METHOD') !== 'POST') {
      show_404();
    ...
    $dashboard_id = $this->apps_model->clone_dashboard($clone_id, $dashboard_name, $note);

    set_alert('success', _l('apps_message_success_clone_dashboard'));
    redirect(admin_url('apps/dashboards/edit_dashboard?dashboard_id=' . $dashboard_id));
  }
  ...
  public function update_dashboard()
  {
    ...
    $this->apps_model->update_dashboard_info($dashboard['id'], $dashboard_name, $note);

    set_alert('success', _l('apps_message_success_update_dashboard'));
    redirect(admin_url('apps/dashboards/edit_dashboard?dashboard_id=' . $dashboard_id));
  }
  ...
  public function update_staff()
  {
    ...
    $this->apps_model->update_dashboard_staff($dashboard_id, $dashboard_staff);

    set_alert('success', _l('apps_message_success_update_dashboard_staff'));
    redirect(admin_url('apps/dashboards/edit_dashboard?dashboard_id=' . $dashboard_id));
  }
  ...
  public function add_widget()
  {
    ...
    $new_widget_rows = $this->apps_model->select_widgets_by_ids([$widget_id]);
    ...
    $this->apps_model->update_dashboard_widgets($dashboard['id'], $dashboard_widgets);

    set_alert('success', _l('apps_message_success_add_widget'));
    redirect(admin_url('apps/dashboards/edit_dashboard?dashboard_id=' . $dashboard_id));
  }
  ...
  public function api_update_widgets_order()
  {
    ...
    $this->apps_model->update_dashboard_widgets($dashboard['id'], $order);
  ...
  {
    if (!has_permission('apps', '', 'dashboard_delete')) {
      access_denied('apps');
    }
    if ($this->input->server('REQUEST_METHOD') !== 'POST') {
      show_404();
    ...
    $this->apps_model->delete_dashboard($dashboard['id']);

    set_alert('success', _l('apps_message_success_delete_dashboard'));
    redirect(admin_url('apps/dashboards'));
  }
  ...
  public function api_available_widgets()
  {
    ...
    $available_widgets = apps_get_available_widgets($dashboard);
