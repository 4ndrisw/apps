...
$fn_get_data = function () {
  ...
  $sql = "
    ...
    FROM
      " . db_prefix() . "staff TBLStaff
          INNER JOIN
      " . db_prefix() . "taskstimers TBLTimers ON TBLStaff.staffid = TBLTimers.staff_id
      INNER JOIN " . db_prefix() . "tasks TBLTasks ON TBLTasks.id = TBLTimers.task_id
          AND TBLTasks.rel_type = 'project'
      INNER JOIN " . db_prefix() . "projects TBLProjects ON TBLProjects.id = TBLTasks.rel_id AND TBLTasks.rel_type = 'project'
  ";
  ...
  foreach ($resultArray as $result_num => $result_row) {
    ...
    array_push($chart['labels'], $result_row['staff_fullname']);
...
<div class="widget widget-projects-logged-hours-by-staff widget-<?= $widget['id'] ?>" data-widget-id="<?= $widget['id'] ?>">
  <div class="row">
    <div class="col-md-12">
      <div class="panel_s">
...
<script>
  window['apps_widget_<?= $widget['id'] ?>_chart_data'] = <?= json_encode($widget_data) ?>;
</script>