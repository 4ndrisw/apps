(function($) {
  ...
  const fnSetupPeriod = async function () {
    var report_from = $(
      '.appsdashboard-period-select input[name="period_from"]'
    );
    var report_to = $('.appsdashboard-period-select input[name="period_to"]');
    var date_range = $(".appsdashboard-date-range");

    $(".appsdashboard-period-dropdown").on("change", function () {
      var val = $(this).val();
      var val_to = "";
      var val_from = "";
    ...
    $(".appsdashboard-period-dropdown").trigger("change");
  };
  ...
  const fnGetLang = async function (langs) {
    return new Promise((resolve, reject) => {
      ...
      $.ajax({
        url: site_url + "admin/apps/api/lang",
        type: "GET",
        data: reqData,
        dataType: "json",
        success: function (rs) {
  ...
  const fnDashboardJs = async function () {
    if (
      typeof window.apps_enable_sortable !== "undefined" ||
      window.apps_enable_sortable
    ) {
      var langs = await fnGetLang(["hide_widget_area", "widget_area"]);

      window.apps_update_widgets_order = function () {
        var order = {};
        ...
        $.ajax({
          url:
            site_url +
            "admin/apps/dashboards/api_update_widgets_order",
          type: "POST",
          data: reqData,
          dataType: "json",
          success: function (rs) {
        ...
        $("[data-container]").sortable({
          ...
          update: function (event, ui) {
            if (this === ui.item.parent()[0]) {
              window.apps_update_widgets_order();
            }
          },
        });
  ...
  const fnPageCategories = async function () {
    var langs = await fnGetLang(["apps_confirm_delete"]);
    ...
    $(".btn-edit-category").on("click", function () {
      ...
      $.ajax({
        url:
          site_url +
          "admin/apps/categories/api_get_category_data?category_id=" +
          id,
        type: "GET",
        data: {},
        dataType: "json",
    ...
    $(".btn-delete").on("click", function () {
      var id = $(this).data("id");
      if (id != undefined && id != null && id != "") {
        if (!confirm(langs.apps_confirm_delete)) {
          return;
        }
  ...
  const fnPageWidgets = async function () {
    ...
    if (widget_edit_permission) {
      $(".widget").prepend(
        `<div class="appsdashboard-widget-removable"></div>`
      );
    }
    if (widget_delete_permission) {
      $(".widget").prepend(
        `<div class="appsdashboard-widget-changeable"></div>`
      );
    }

    // on remove widget
    $("body").on("click", ".appsdashboard-widget-removable", function () {
      if (!confirm(langs.apps_confirm_delete)) {
        return;
      }
    ...
    var apps_lock_changeable = false;
    $("body").on("click", ".appsdashboard-widget-changeable", function () {
      if (apps_lock_changeable) {
        return;
      }
      ...
      apps_lock_changeable = true;
      $.ajax({
        url:
          site_url +
          "admin/apps/widgets/api_get_widget_data?widget_id=" +
          id,
        type: "GET",
        data: {},
        dataType: "json",
  ...
  const fnPageEditDashboard = async function () {
    var langs = await fnGetLang([
      ...
      "apps_confirm_delete",
      "not_found",
      "apps_message_success_delete_widget_from_dashboard",
    ]);

    window.apps_enable_sortable = true;
    fnDashboardJs();

    var widget_preview_url =
      site_url + "admin/apps/widgets?category=&search=";
    // add widget
    (function () {
      $("#formAddWidget").on(
        "click",
        ".appsdashboard-add-widget",
        function () {
          var widgetId = $(this).data("widget-id");
    ...
    (function () {
          ...
          availables.forEach((available) => {
            wrapper.append(`
              <tr>
                ...
                <td><a class="appsdashboard-add-widget" href="javascript:void(0);" data-widget-id="${
                  available.id
                }" data-widget-container="${available.default_container}">${
              langs["add"]
            }</a></td>
                <td><a class="appsdashboard-add-widget" href="${
      ...
      window.apps_fetch_available_widgets = function () {
        var reqData = {
          dashboard_id: DASHBOARD_ID,
        };
        reqData[csrfData.token_name] = csrfData.hash;
        $.ajax({
          url:
            site_url +
            "admin/apps/dashboards/api_available_widgets",
          type: "POST",
          data: reqData,
          dataType: "json",
          success: function (rs) {
      ...
      window.apps_fetch_available_widgets();
    })();
    ...
    (function () {
      ...
      $("[data-container]").on(
        "click",
        ".appsdashboard-widget-removable",
        function () {
          if (!confirm(langs.apps_confirm_delete)) {
            return;
          }
          ...
          alert_float(
            "warning",
            langs.apps_message_success_delete_widget_from_dashboard
          );
        }
      );
    })();
