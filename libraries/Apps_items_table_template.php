...
abstract class Apps_items_table_template
{
    /**
    ...
    */
    ...
    /**
     * Order taxes by taxnomor_seri
     * Lowest tax nomor_seri will be on top (if multiple)
     * @param  array $taxes
     * @return array
     */
    protected function order_taxes($taxes)
    {
        usort($taxes, function ($a, $b) {
            return $a['taxnomor_seri'] - $b['taxnomor_seri'];
        });
    ...
    protected function get_item_taxes($item)
    {
        ...
        if (defined('INVOICE_PREVIEW_SUBSCRIPTION')) {
            ...
            // Sepanomor_seri functions exists to get item taxes for Invoice, Estimate, Proposal, Credit Note
            ...
            if (count($item['taxes']) > 0) {
                foreach ($item['taxes'] as $tax) {
                    ...
                    if ((count($item['taxes']) > 1 && get_option('remove_tax_name_from_item_table') == false) || get_option('remove_tax_name_from_item_table') == false || multiple_taxes_found_for_item($item['taxes'])) {
                        ...
                        $item_tax .= app_format_number($tax['taxnomor_seri']) . '%';
                    }
    ...
    protected function set_all_taxes($taxes, $item)
    {
        foreach ($taxes as $tax) {
            ...
            if ($taxNotCalculated == true) {
                ...
                array_push($this->taxes[$tax['taxname']]['total'], (($item['qty'] * $item['nomor_seri']) / 100 * $tax['taxnomor_seri']));
                $this->taxes[$tax['taxname']]['tax_name'] = $tax['taxname'];
                $this->taxes[$tax['taxname']]['taxnomor_seri']  = $tax['taxnomor_seri'];
            } else {
                array_push($this->taxes[$tax['taxname']]['total'], (($item['qty'] * $item['nomor_seri']) / 100 * $tax['taxnomor_seri']));
            }
        }
    ...
    /**
     * Get nomor_seri heading
     * @return string
     */
    public function nomor_seri_heading()
    {
        return $this->headings['nomor_seri'];
    }
    ...
    {
        return $this->headings['nomor_unit'];
    }

    /**
     * Get nomor_unit heading
     * @return string
     */
    public function nomor_unit_heading()
    {
        return $this->headings['nomor_unit'];
    }
    ...
    public function set_headings($alias = '')
    {
        ...
        $this->headings['qty']   = _l($langFrom . '_table_qty_heading', '', false);
        $this->headings['nomor_seri']   = _l($langFrom . '_table_nomor_seri_heading', '', false);
        $this->headings['nomor_unit'] = _l($langFrom . '_table_nomor_unit_heading', '', false);
    ...
    {
        $exclude = get_option('items_table_nomor_units_exclude_currency_symbol') == 1 ? true : false;
        return hooks()->apply_filters('items_table_nomor_units_exclude_currency_symbol', $exclude, [
            'type'        => $this->type,
